...
namespace deva {
        ...
        behavior::ExecuteResult InLineOfSightNode::Execute(behavior::ExecuteContext& ctx) {
            ...
            if (!center.hit) {
                ...
                if (target_player) {
                       ...
                       // if (ctx.com->ShootWall(target_player->position, target_player->velocity, game.GetPlayer().GetHeading(), bullet_speed, bullet_time, 10)) {
        ...
        behavior::ExecuteResult LookingAtEnemyNode::Execute(behavior::ExecuteContext& ctx) {
            ...
            Vector2f seek_position = ctx.com->CalculateShot(game.GetPosition(), target.position, bot_player.velocity, target.velocity, proj_speed);

            Vector2f totarget = seek_position - game.GetPosition();
            ...
            if ((game.GetShipSettings().DoubleBarrel & 1) != 0) {
                ...
                bool hit1 = RayBoxIntersect(bot_player.position + side * radius, bot_player.GetHeading(), box_min, box_extent, &dist, &norm);
                bool hit2 = RayBoxIntersect(bot_player.position - side * radius, bot_player.GetHeading(), box_min, box_extent, &dist, &norm);
                if (hit1 || hit2) { hit = true; }
           ...
           // }
            ...
            if (seek_position.DistanceSq(target_player->position) < 15 * 15) {
                ctx.blackboard.Set("shot_position", seek_position);
            }
            else {
               // ctx.blackboard.Set("shot_position", target.position);
                ctx.blackboard.Set("shot_position", seek_position);
            }                    
...
//#endif
            ...
            ctx.deva->Move(shooter.position, hover_distance);
