...
namespace marvin {
	...
	void SendCommandsMenu(GameProxy& game, std::string player) {
		game.SendChatMessage(":" + player + ":Format: enable command/disable command {shorthand command} -- description (accepted chat type) [restrictions]");
		game.SendChatMessage(":" + player + ":");
		game.SendChatMessage(":" + player + ":!lockmarv/!unlockmarv {.lm/.um} -- only mods can send bot commands (any) [mod only]");
		game.SendChatMessage(":" + player + ":!lockfreq/!unlockfreq {.lf/.uf} -- disable/enable the bots freq manager (any) [devastation]");
		game.SendChatMessage(":" + player + ":");
		...
		game.SendChatMessage(":" + player + ":!swarm/!swarmoff {.s/.so} -- bots will respawn quickly with low health when basing (any) [mod only]");
		game.SendChatMessage(":" + player + ":!repel/!repeloff {.rep/.ro} -- use repels in basing behavior when rushing (any)");
		game.SendChatMessage(":" + player + ":");
