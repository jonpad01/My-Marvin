...
namespace marvin {
    ...
    uint64_t Time::DevaFreqTimer(std::vector<std::string> names) {
        ...
        //as long as the list of names stays in the same order, each bot will return a unique offset depending on where its name is in the list
        ...
        //after the loop has completed, if the special state was found, make bots that are basing wait the maximum offset limit 
        //until the bots that arent basing jump in
        if (wait_for_bot) {
            offset += offset_index * 200;
            if (bot_dead) {
