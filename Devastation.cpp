...
//#endif
        ...
        behavior::ExecuteResult BundleShots::Execute(behavior::ExecuteContext& ctx) {
            ...
            const Player& target = *ctx.blackboard.ValueOr<const Player*>("target_player", nullptr);
        ...
        behavior::ExecuteResult MoveToEnemyNode::Execute(behavior::ExecuteContext& ctx) {
            ...
            uint64_t time = ctx.bot->GetTime();
            uint64_t energy_check_delay = 25;
            uint64_t emp_cooldown = 1000;
...
//#if 0
            ...
            for (Weapon* weapon : game.GetWeapons()) {
                ...
                if (mine_position.Distance(game.GetPosition()) < 5.0f && bot_is.in_center) {
...
//#endif
            ...
            if (time > energy_time_check) {
                ...
                ctx.blackboard.Set("EnergyTimeCheck", time + energy_check_delay);
                ctx.blackboard.Set("EnergyCheck", energy);
                if (energy == energy_check && energy_pct < 0.95f) {
                    ctx.blackboard.Set("Emped", true);
                    ctx.blackboard.Set("EmpCooldown", time + emp_cooldown);
                }
            }
